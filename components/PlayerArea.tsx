...
const PlayerArea: React.FC<PlayerAreaProps> = ({ 
  ...
  const containerClasses = {
    bottom: "flex-col-reverse bottom-4 left-1/2 -translate-x-1/2",
    top: "flex-col top-4 left-1/2 -translate-x-1/2",
    left: "flex-row top-1/2 left-4 -translate-y-1/2",
    right: "flex-row-reverse top-1/2 right-4 -translate-y-1/2",
  };

  const handContainerClasses = {
    bottom: "flex gap-2 justify-center mt-4",
  ...
  return (
    <div className={`absolute flex items-center ${containerClasses[position]} transition-all duration-500`}>
      ...
      {/* Info Bubble */}
      <div className={`
        relative px-4 py-2 rounded-xl backdrop-blur-md shadow-xl border
      ...
      `}>
        ...
        {isActive && <div className="text-xs text-yellow-300 mt-1 font-semibold animate-pulse">Thinking...</div>}
        ...
        {player.lastAction && (
          <div className="absolute -top-8 bg-white text-slate-900 text-xs px-2 py-1 rounded-full shadow-lg whitespace-nowrap animate-bounce">
            {player.lastAction}
          </div>
        )}
      </div>
